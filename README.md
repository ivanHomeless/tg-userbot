...
- ✅ **Надежная** обработка альбомов через Telethon `events.Album`
- ✅ Автоматический сбор всех текстов из альбома
- ✅ Умное ожидание текста для одиночного медиа (20 секунд)
...
- **Надежная** обработка альбомов через `events.Album` (Telethon собирает все медиа)
- Сбор **всех** текстов из всех сообщений альбома
- Система ожидания текста для одиночного медиа (20 секунд)
...
└────┬────────────┘
     ...
     └─► PostBuilder (каждые 3 сек)
         └─ Сборка постов (быстрая обработка альбомов)
                       │
                       ▼
                ┌──────────────┐
...
**Два обработчика:**
- `collect_album(event)` - для `events.Album()` (Telethon собрал все медиа)
- `collect_message(event)` - для `events.NewMessage()` (одиночные сообщения)
...
- Медиа без текста (одиночное) → `awaiting_text=True` (20 сек)
...
3. **PostBuilder** (3 сек)
   - Собирает альбомы **сразу** (Telethon уже дождался всех медиа)
...
- ✅ Фильтрация собственных сообщений из DEST канала

**Технические детали:**
- Два обработчика: `album_handler` и `message_handler`
- Новый метод `MessageCollector.collect_album()`
- Удалены: `_album_locks`, `_album_timers`, `ALBUM_BUILD_TIMEOUT`
