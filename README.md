...
- ✅ Обработка **альбомов** (media groups) с единым рерайтингом
- ✅ Умное ожидание текста для медиа (10 секунд)
...
- Автоматическая обработка альбомов
- Система ожидания текста для медиа без подписи
...
- Обработка альбомов с единым caption
- Сборка постов из обработанных сообщений
- Контроль таймаутов для альбомов
...
└────┬────────────┘
     ...
     │
     └─► PostBuilder (каждые 45 сек)
         └─ Сборка постов из готовых сообщений
                       │
                       ▼
                ┌──────────────┐
...
- Медиа с текстом (альбом) → `rewrite_status=skipped`, рерайт в PostBuilder
- Медиа без текста (одиночное) → `awaiting_text=True` (10 сек)
- Медиа без текста (альбом) → `rewrite_status=skipped`
...
3. **PostBuilder** (45 сек)
   - Собирает альбомы (если `collected_at` старше 10 сек)
   - Рерайтит caption альбома через AI
...
# Настройки обработки
AWAIT_TEXT_TIMEOUT=10      # Таймаут ожидания текста для медиа (секунды)
```
...
**Методы:**
- `collect_message(event)` - обработка нового сообщения
- `handle_text_message(msg, chat_id)` - текст без медиа
- `handle_media_with_text(msg, chat_id)` - медиа + текст
- `handle_media_without_text(msg, chat_id)` - медиа без текста
- `_update_album_collected_at(chat_id, grouped_id)` - обновление таймера альбома
